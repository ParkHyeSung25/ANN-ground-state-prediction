# -*- coding: utf-8 -*-
"""2021_8_30_O(16)_atom_nucleus_woods_saxon_potential.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/11SyQYm3bnQNfilRBCzgGXe6vtYHEJCdz
...
class HydrogenAtom(object):
    ...
    def __init__(self, sampler):
        ...
        self.hbaromega = tf.constant(16.0 , dtype = DEFAULT_TENSOR_TYPE2)*self.MeV
        ...
        # Potential constant
        self.V0 = tf.constant(78.0*MeV, dtype = DEFAULT_TENSOR_TYPE2)
        self.R0 = tf.constant(2.4*fm, dtype = DEFAULT_TENSOR_TYPE2)
        self.Rc = tf.constant(2.1*fm, dtype = DEFAULT_TENSOR_TYPE2)
        self.Valpha = tf.constant(2.064, dtype = DEFAULT_TENSOR_TYPE2)
        self.a = tf.constant(0.5*fm, dtype = DEFAULT_TENSOR_TYPE2)
        self.ac = tf.constant(0.1*fm, dtype = DEFAULT_TENSOR_TYPE2)
        ...
        if self.npart > 1:
          ...
          for i in range(self.npart-1):
            ...
            for j in range(i+1, self.npart):
              ...
              del_r2 = tf.reduce_sum(walker_dnm**2, axis=(1))
############################################################################################################################################################
              for d in range(500):
                del_ri = del_r[d]
                delr2i = del_r2[d]
                #print("이게 500,?...",del_ri)
                if del_ri >= self.Rc:
                  Vri = - self.V0 * ( 1.0/(1+tf.exp((del_ri - self.R0)/self.a)) - (self.Valpha * (1.0/(1+tf.exp((del_ri - self.Rc)/self.ac)))))
                  ainteraction = self.alpha*self.hbarc*4.0*(1. /(del_ri+1e-8))
                  Vvec = Vvec + Vri + ainteraction
                else :
                  Vri = - self.V0 * ( 1.0/(1+tf.exp((del_ri - self.R0)/self.a)) - (self.Valpha * (1.0/(1+tf.exp((del_ri - self.Rc)/self.ac)))))
                  ainteraction = self.alpha*self.hbarc*4.0*((1.0/(2.0*self.Rc)) * (3.0 - (delr2i / (self.Rc**2))))
                  Vvec = Vvec + Vri + ainteraction
              totvri = Vvec/500
              totvr = totvr + totvri
              Vvec = 0
