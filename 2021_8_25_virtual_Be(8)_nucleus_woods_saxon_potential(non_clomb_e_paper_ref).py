# -*- coding: utf-8 -*-
"""2021_8_25_virtual_Be(8)_nucleus_woods_saxon_potential(non_clomb_e_paper_ref).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ZxYJtIBJ9n67rMlTEyqqY4Daq2O_GAox
...
class HydrogenAtom(object):
    ...
    def __init__(self, sampler):
        ...
        # Potential constant
        self.V0 = tf.constant(53.8*MeV, dtype = DEFAULT_TENSOR_TYPE2)
        self.R0 = tf.constant(2.4*fm, dtype = DEFAULT_TENSOR_TYPE2)
        self.Rc = tf.constant(2.1*fm, dtype = DEFAULT_TENSOR_TYPE2)
        self.Valpha = tf.constant(3.0, dtype = DEFAULT_TENSOR_TYPE2) #2.064
    ...
    def interaction_energy(self, *, logw_of_x, inputs):
        ...
        if self.npart > 1:
          for ii in range(self.npart):
              ...
              iwalkers = tf.split(inputs, self.npart, axis=(1))[ii]
              walker_dn = tf.reduce_sum(iwalkers, axis=(1))
              r = tf.sqrt(tf.reduce_sum(walker_dn**2, axis=(1)))

              Vri = - self.V0 * ( 1.0/(1+tf.exp((r - self.R0)/self.a)) - (self.Valpha * (1.0/(1+tf.exp((r - self.Rc)/self.ac)))))
              Vr = Vr + Vri
